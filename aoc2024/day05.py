...
def load_data(file_name):
    ...
    rules_dict = {
        page: [b for a, b in rules if a == page] for page in set(x for x, _ in rules)
    }
    updates = (
        list(map(int, line.split(","))) for line in update_data.strip().split("\n")
    )
...
def is_valid_update(update, rules):
    return all(
        all(r not in update[:i] for r in rules.get(page, []))
...
def correct_updates(updates, rules):
    return (
        sorted(update, key=lambda page: PageCompare(page, rules)) for update in updates
    )
...
if __name__ == "__main__":
    ...
    run(part_1, "data/day05-example.txt", expected=143)
    run(part_1, "data/day05-data.txt", expected=5329)
    run(part_2, "data/day05-example.txt", expected=123)
    run(part_2, "data/day05-data.txt", expected=5833)
